...
for i in [0,1,2,3]:
    ...
    for angle in angle_list:
        ...
        sample_random = preprocess.DataSampler(np.zeros((n_points,)).astype(int), random_seed=100, verbose=False)
        (idx_train, idx_test) = sample_random.random(80)

        tf.random.set_seed(random_seed)

        input_X = keras.Input(shape=(1,))
        decoded = layers.Dense(1, activation='linear', kernel_initializer=initializers.glorot_uniform(seed=random_seed), bias_initializer='zeros')(input_X)
    ...
    np.savetxt('synthetic-dataset-linear-reconstruction-from-subspace-phi-' + str(i+1) + '-MSE-loss.csv', (MSE_losses), delimiter=',', fmt='%.16e')
